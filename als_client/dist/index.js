 ...
 */
async function getBotUsername(scraper) {
    try {
        const me = await scraper.me();
 ...
 */
async function generateUniqueQuote(tweetContent, responseText) {
    try {
 ...
 */
async function optimizeForTwitter(responseText, tweetContent) {
    try {
        if (!process.env.OPENAI_API_KEY) {
 ...
 */
async function sendDirectMessage(scraper, username, messageText) {
    try {
...
async function processTweetAndRespond(scraper, tweet) {
    try {
        ...
        if (tweet.response) {
            ...
            const dmSent = await sendDirectMessage(scraper, tweet.username, `Hi there! Here's my response to your tweet that mentioned ${SEARCH_KEYWORD}:\n\n${tweet.response}\n\nI'll also post a shorter version as a public reply.`);
            tweet.dmSent = dmSent;
            if (dmSent) {
            ...
            try {
                ...
                try {
                    isAuthenticated = await Promise.race([
                        scraper.isLoggedIn(),
                        new Promise((_, reject) => setTimeout(() => reject(new Error('Login check timed out')), 15000))
                    ]);
                }
